...
- [Orbi](https://github.com/orbi-build/orbi) - GitHub Issue駆動のセルフホスト型自律コーディングエージェント。Issueに `ai-ready` ラベルを付けると、独立したworktreeで実装してPRを作成し、別セッションのレビューがIssueの受け入れ条件と照合し、レビュー済みのheadだけをマージしてタグ付きリリースを作成する。Codex、DeepSeek、GLM、ローカルモデルで動作。オープンソース（AGPL-3.0。Sustainable Use Licenseも選択可）
...
- [DevProjex](https://github.com/Avazbek22/DevProjex) - GUI、TUI、CLI と組み込みの読み取り専用 MCP サーバーを備えたクロスプラットフォームな .NET/Avalonia コードベースコンテキストツールで、制御された token-efficient なコンテキスト作成に対応します
...
- [Winx Code Agent](https://github.com/gabrielmaialva33/winx-code-agent) - WCGWとSerenaの最高機能を組み合わせた最大効率・セマンティック機能向けRust記述高性能コードエージェント
...
- [DeepFabric](https://github.com/nolabs-ai/deepfabric) - 構造化出力とより効率的なツール呼び出しのためのモデルファインチューニング用合成データセット、およびエージェントとモデル評価データの生成
